...
with open(file_path, 'r') as file:
    lines = file.readlines()[7:]  # Skip first 7 lines
    ...
    for line in lines:
        ...
        if len(parts) >= 6:  # Must have at least 6 columns (NTIME ITER U V P ...)
            try:
                ...
                rest_val = float(parts[4])        # Residual T
...
print(f"Lengths -> NTIME: {len(ntime)}, ResU: {len(res_u)}, ResV: {len(res_v)}, ResT: {len(res_t)}")
